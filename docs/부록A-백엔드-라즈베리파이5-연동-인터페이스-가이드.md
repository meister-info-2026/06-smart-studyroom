...
| `pip install lgpio` C 컴파일 오류 / 빌드 실패 | PyPI의 lgpio C 소스 빌드 환경 문제 | `sudo apt install -y python3-gpiozero python3-lgpio`로 라즈베리파이 OS 기본 패키지를 설치하고, `python3 -m venv --system-site-packages venv`로 가상환경을 생성하여 재사용 |
| gpiozero `BadPinFactory` 등 핀 제어 실패 | Pi 5(RP1 칩)에 `lgpio` 미설치 또는 핀 팩토리 미지정 | 시스템 패키지(`python3-lgpio`) 설치 및 `--system-site-packages` venv 사용 확인, `pi/.env`의 `GPIOZERO_PIN_FACTORY=lgpio` 설정 확인 |
